...
if len(sys.argv) != 15:
	sys.exit("Usage: python make_cross_section_plot.py PLANE1XTR PLANE2XTR STEPLENGTH1 STEPLENGTH2 TIME SCALEA SCALEB MINEXISTENT ANNOTATE NORMAL{'None', or comma separated list} TRANSLATE{'None', or comma separated list} RELATIVE{'T' or 'F'} NORMVEL{'T' or 'F'} OUTPUTFILENAME")
...
translate = sys.argv[11]
if translate == "None":
	translate = ""
else:
	translate = " --translate " + translate
relative = sys.argv[12]
if relative == 'T':
...
if normvel == 'T':
	...
	sys.exit("NORMVEL should be 'T' or 'F'")
...
execute(hemeXtract + " -C " + plane1 + " " + plane2 + " -A " + steplength1 + " -B " + steplength2 + " -1 " + time + " --scaleA " + scaleA + " --scaleB " + scaleB + " --minexistent " + minexistent + normal + translate + relative + " --quiet -n 1 -o /tmp/__hemeXtract_output\n")
...
if len(relative) > 0:
	...
	if normvel == True:
		...
		plt.colorbar(label=r'Vel. Diff. (ms$^{-1}$)')
else:
#	levels = MaxNLocator(nbins=15).tick_values(out_err.min(), out_err.max())
#	plt.contourf(xi, yi, zi, origin='lower', extent=[0.93*(out_x.min()), 1.07*(out_x.max()), 0.93*(out_y.min()), 1.07*(out_y.max())], cmap=plt.cm.inferno, levels=levels)
