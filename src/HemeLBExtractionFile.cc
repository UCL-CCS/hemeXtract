...
bool HemeLBExtractionFile::read_time_next()
{
...
SiteIndex * HemeLBExtractionFile::get_site_indices(Site *list, uint64_t list_size)
{
	return snapshot->get_site_indices(list, list_size);
}

SiteIndex * HemeLBExtractionFile::get_site_indices_hashed_lookup(Site *list, uint64_t list_size)
{
...
int HemeLBExtractionFile::read_header()
{
...
int HemeLBExtractionFile::read_field_header()
{
	...
	// Calculate number of 'columns' needed to represent a snapshot
