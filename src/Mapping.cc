...
lattice_map * get_mapping(HemeLBExtractionFile *A, HemeLBExtractionFile *B)
{
	...
	//	return NULL;
	//}
	std::vector<Site> mapped_sites(num_sites_A * 8);
	...
	SiteIndex *site_indices = B->get_site_indices_hashed_lookup(
		mapped_sites.data(), num_sites_A * 8);
